...
def show_history(session_state):
    if app.use_chat_history and session_state["chat_history"] is not None:
        try:
            ...
            logger.info(f"DEBUG: Error retrieving chat history: {str(e)}")
...
def get_response(
    ...
    with get_openai_callback() as cb:
        ...
        if route:
            logger.info(f"Semantic Router Hit: {route}")
        ...
        if app.use_chat_history and session_state["chat_history"] is not None:
            ...
            logger.info("DEBUG: Chat history not updated (disabled or None)")
    ...
    with gr.Blocks(
        ...
        with gr.Row():
            ...
            with gr.Column(scale=6):
                ...
                with gr.Accordion(
                    label="Embedding Model Settings",
                    open=True,
                    elem_classes=["accordion"],
                ):
                    with gr.Row():
                ...
                with gr.Row():
                    select_pdf_btn = gr.Button(
                        "📄 Select PDF", elem_id="select-pdf-btn"
                    )
        ...
        blocks.load(check_credentials, outputs=credentials_modal)
